...
describe("Settlement", async () => {
	beforeEach(async () => {
		...
		if (network.name === "mainnet") sovrynSwapNetworkAdr = "0x98ace08d2b759a265ae326f010496bcd63c15afc";
			...
			sovrynSwapNetworkAdr = (await get("TestSovrynSwap")).address;
	...
	it("Should createOrder()", async () => {
		...
		const { order } = await createOrder(users[0], fromToken, toToken, parseEther("100"), parseEther("0.005"), getDeadline(24));
	...
	it("Should fillOrder()", async () => {
		const { users, fillOrder, filledAmountIn } = await helpers.setup();
		const { abi: settlementABI } = await deployments.get("SettlementLogic");
		const { address: settlementAddress } = await deployments.get("SettlementProxy");
		const settlement = await ethers.getContractAt(settlementABI, settlementAddress);
	...
	it("Should fillOrder() RBTC-XUSD", async () => {
		const { users, fillOrder, filledAmountIn, chainId } = await helpers.setup();
		...
		const filled = settlement.interface.decodeEventLog("OrderFilled", event.data, event.topics);
		console.log(
			"Order filled: %sWRBTC -> %sXUSD",
			formatEther(filled.amountIn.toString()),
			formatUnits(filled.amountOut.toString(), 18)
		);
		await helpers.expectToEqual(filled.hash, orderG.hash());
		await helpers.expectToEqual(filled.amountIn, getActualFillAmount(orderG));
	});
	...
	it("Should createOrder() XUSD-RBTC", async () => {
		...
		const { order, tx } = await createOrder(users[0], fromToken, toToken, parseEther("100"), parseEther("0.00001"), getDeadline(24));
	...
	it("Should fillOrder() XUSD-RBTC", async () => {
		...
		const filled = settlement.interface.decodeEventLog("OrderFilled", event.data, event.topics);
		console.log(
			"Order filled: %sXUSD -> %sRBTC",
			formatUnits(filled.amountIn.toString(), 18),
			formatEther(filled.amountOut.toString())
		);
		await helpers.expectToEqual(filled.hash, orderG.hash());
		await helpers.expectToEqual(filled.amountIn, getActualFillAmount(orderG));
	});
	...
	it("Should cancelOrder()", async () => {
		...
		const { order, tx } = await createOrder(users[0], fromToken, toToken, parseEther("0.01"), parseEther("0.0001"), getDeadline(24));
	...
	it("Should failed fillOrder because minFee", async () => {
		...
		const { order, tx } = await createOrder(users[0], fromToken, toToken, parseEther("0.002"), parseEther("0.0001"), getDeadline(24));
		await tx.wait();
	...
	it("Should withdraw RBTC when cancel order", async () => {
		...
		const { order } = await createOrder(signer, fromToken, toToken, amountIn, amountOutMin, getDeadline(24));
	...
	it("Should fill order partial", async () => {
		...
		const { order, tx } = await createOrder(users[0], fromToken, toToken, parseEther("10000"), parseEther("5000"), getDeadline(24));
		await tx.wait();

		const sovrynSwapNetwork = await ethers.getContractAt(sSNAbi, sovrynSwapNetworkAdr, users[0]);
		const path = await sovrynSwapNetwork.conversionPath(fromToken.address, toToken.address);

		const fillAmount1 = parseEther("4800"),
			fillAmount2 = parseEther("5100");
